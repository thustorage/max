...
void f2fs_submit_page_mbio(struct f2fs_io_info *fio) {
	...
	enum page_type btype = PAGE_TYPE_OF_BIO(fio->type); // DATA, NODE, META
	struct f2fs_bio_info *io;
	...
	if (bio_add_page(io->bio, bio_page, PAGE_CACHE_SIZE, 0) <
		PAGE_CACHE_SIZE) { // when bio is full
		__submit_merged_bio(io);
		goto alloc_new;
	}
...
int f2fs_reserve_block(struct dnode_of_data *dn, pgoff_t index) {
	...
	err = get_dnode_of_data(dn, index, ALLOC_NODE); // node info for index'th data
	if (err)
		return err;
...
static int f2fs_write_data_page(struct page *page,
	...
	write:
	if (unlikely(is_sbi_flag_set(sbi, SBI_POR_DOING))) // doing recovery
		goto redirty_out;
	...
	err = -EAGAIN;
	f2fs_lock_op(sbi); // gains a read semaphore
	if (f2fs_has_inline_data(inode))
...
static int f2fs_write_data_pages(struct address_space *mapping,
	...
	diff = nr_pages_to_write(sbi, DATA, wbc); // SYNC_ALL:0;SYNC_NONE:DATA4096
	if (!S_ISDIR(inode->i_mode)) {
...
static int f2fs_write_begin(struct file *file, struct address_space *mapping,
	...
	f2fs_balance_fs(sbi); // do gc if has not enough free space
	...
	repeat:
	page = grab_cache_page_write_begin(mapping, index, flags); // data page
	if (!page) {
		err = -ENOMEM;
		goto fail;
	}
	...
	*pagep = page;
	...
	err = f2fs_reserve_block(&dn, index); // allocate first PAGE(in pos) for a file, just set data_addr to NEW_ADDR
	if (err)
		goto put_fail;
	put_next:
	f2fs_put_dnode(&dn);
	f2fs_unlock_op(sbi); // release a cp_rwsem

	if ((len == PAGE_CACHE_SIZE) ||
		PageUptodate(page)) // data address has been reset previously in f2fs_reserve_block(&dn,index)
		return 0;
	...
	if (dn.data_blkaddr == NEW_ADDR) { // overwrite
		zero_user_segment(page, 0, PAGE_CACHE_SIZE);
	} else { // read from ssd first
		struct f2fs_io_info fio = {
				.sbi = sbi,
				.type = DATA,
				.rw = READ_SYNC,
...
#ifdef FILE_CELL
	if ((inode->i_ino == F2FS_ROOT_INO(sbi) || inode->i_ino > F2FS_RESERVED_NODE_NUM) &&
		(offset % PAGE_CACHE_SIZE || length != PAGE_CACHE_SIZE))
		return;
#else
	if (inode->i_ino >= F2FS_ROOT_INO(sbi) && (offset % PAGE_CACHE_SIZE || length != PAGE_CACHE_SIZE))
	return;
#endif
	if (PageDirty(page)) {
...
#ifdef FILE_CELL
		else if (F2FS_IS_NODE(sbi, inode->i_ino))
			dec_dirty_node_page_count(sbi, NODE_IDX(inode->i_ino, sbi));
#else
