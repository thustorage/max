...
#ifdef FILE_CELL
	if (F2FS_IS_NODE(sbi, ino) || ino == F2FS_META_INO(sbi))
		goto make_now;
#else
...
#ifdef FILE_CELL
	if (F2FS_IS_NODE(sbi, inode->i_ino) || inode->i_ino == F2FS_META_INO(sbi))
		return 0;
#else
...
#ifdef FILE_CELL
	invalidate_mapping_pages(NODE_MAPPING(sbi, inode->i_ino), inode->i_ino, inode->i_ino);
#else
