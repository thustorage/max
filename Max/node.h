...
#define LIST_IDX(nid, nm_i)  ((nid) / (nm_i)->nid_chunk)
#endif
...
static inline void next_free_nid(struct f2fs_sb_info *sbi, nid_t *nid) {
	...
	*nid = NAT_ENTRY_PER_BLOCK;
	// find minimal free nid
//	int nid_list_cnt = nm_i->nid_list_count;
//	int i;
//	nid_t min = nm_i->max_nid;
//	for (i = 0; i < nid_list_cnt; i++) {
//		spin_lock(&nm_i->free_nid_list_lock[i]);
//		if (nm_i->percore_fcnt[i] <= 0) {
//			spin_unlock(&nm_i->free_nid_list_lock[i]);
//			continue;
//		}
//		fnid = list_entry(nm_i->free_nid_list[i].next, struct free_nid, list);
//		if (fnid->nid < min) {
//			min = fnid->nid;
//		}
//		spin_unlock(&nm_i->free_nid_list_lock[i]);
...
static inline unsigned int sum_up_fcnt(struct f2fs_nm_info *nm_i) {
	unsigned int count = 0;
	int nid_list_cnt = nm_i->nid_list_count;
	int i;
	for (i = 0; i < nid_list_cnt; i++) {
		count += nm_i->percore_fcnt[i];
	}
	return count;
}
