...
#ifdef MLOG
	MSG(0, "  -N number of mlogs [default:1]\n");
#endif
...
{
	static const char *option_string = "qa:c:d:e:l:mo:O:s:z:t:N:";
	int32_t option=0;
...
#ifdef MLOG
		case 'N':
			c.nr_mlog = atoi(optarg);
			if (c.nr_mlog <=0 ) c.nr_mlog = 1;
			if (c.nr_mlog > 64) {
				MSG(0, "Max supported mlogs 64\n");
				c.nr_mlog = 64;
			}
			break;
#endif
