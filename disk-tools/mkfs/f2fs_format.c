...
#ifdef MLOG
	set_cp(nr_mlog, c.nr_mlog);
	u_int32_t meta_zone = c.cur_seg[CURSEG_HOT_DATA], data_zone = c.cur_seg[CURSEG_WARM_DATA];
	if (c.heap) {
		for (i = 1; i < c.nr_mlog; i++) {
			meta_zone -= c.segs_per_zone;
			set_cp(cur_node_segno[3 * i + 0], meta_zone); // hot node
			meta_zone -= c.segs_per_zone;
			set_cp(cur_node_segno[3 * i + 1], meta_zone); // warm node
			meta_zone -= c.segs_per_zone;
			set_cp(cur_node_segno[3 * i + 2], meta_zone); // cold node
			meta_zone -= c.segs_per_zone;
			set_cp(cur_data_segno[3 * i + 0], meta_zone); // hot data
			data_zone += c.segs_per_zone;
			set_cp(cur_data_segno[3 * i + 2], data_zone); // cold data
			data_zone += c.segs_per_zone;
			set_cp(cur_data_segno[3 * i + 1], data_zone); // warm data
		}
		for (i = 0; i < c.nr_mlog; i++) {
			MSG(0, "mlog %d: %u, %u, %u, %u, %u, %u\n", i, get_cp(cur_node_segno[3 * i + 0]), get_cp(cur_node_segno[3 * i + 1]), get_cp(cur_node_segno[3 * i + 2]), get_cp(cur_data_segno[3 * i + 0]), get_cp(cur_data_segno[3 * i + 1]), get_cp(cur_data_segno[3 * i + 2]));
		}
	} else {
		for (i = 1; i < c.nr_mlog; i++) {
			data_zone += c.segs_per_zone;
			set_cp(cur_node_segno[3 * i + 0], data_zone); // hot node
			data_zone += c.segs_per_zone;
			set_cp(cur_node_segno[3 * i + 1], data_zone); // warm node
			data_zone += c.segs_per_zone;
			set_cp(cur_node_segno[3 * i + 2], data_zone); // cold node
			data_zone += c.segs_per_zone;
			set_cp(cur_data_segno[3 * i + 0], data_zone); // hot data
			data_zone += c.segs_per_zone;
			set_cp(cur_data_segno[3 * i + 1], data_zone); // cold data
			data_zone += c.segs_per_zone;
			set_cp(cur_data_segno[3 * i + 2], data_zone); // warm data
		}
	}
	for (i = c.nr_mlog * 3 ; i < MAX_ACTIVE_NODE_LOGS + MAX_EXTEND_LOGS * NR_CURSEG_NODE_TYPE; i++) {
...
#ifdef MLOG
	set_cp(cp_pack_total_block_count, 6 + get_sb(cp_payload) + 6 * (c.nr_mlog - 1));
#else
...
#ifdef FILE_CELL
	set_cp(next_free_nid, NAT_ENTRY_PER_BLOCK); // for Max
#else 
...
#ifdef MLOG 
	// compact summary block only for mlog 0, the following mlogs use nornal summary block
	// 6 normal summary blocks for each mlog
	for (i = 1; i < c.nr_mlog; i++) {
		/* HOT_DATA */
		memset(sum, 0, sizeof(struct f2fs_summary_block));
		SET_SUM_TYPE((&sum->footer), SUM_TYPE_DATA);
		cp_seg_blk++;
		DBG(1, "\tWriting Segment summary for mlog %d HOT_DATA, at offset 0x%08"PRIx64"\n", i, cp_seg_blk);
		if (dev_write_block(sum, cp_seg_blk)) {
			MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
			goto free_cp_payload;
		}
		...
		SET_SUM_TYPE((&sum->footer), SUM_TYPE_DATA);
		cp_seg_blk++;
		DBG(1, "\tWriting Segment summary for mlog %d WARM_DATA, at offset 0x%08"PRIx64"\n", i, cp_seg_blk);
		if (dev_write_block(sum, cp_seg_blk)) {
			MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
			goto free_cp_payload;
		}
		...
		SET_SUM_TYPE((&sum->footer), SUM_TYPE_DATA);
		cp_seg_blk++;
		DBG(1, "\tWriting Segment summary for mlog %d COLD_DATA, at offset 0x%08"PRIx64"\n", i, cp_seg_blk);
		if (dev_write_block(sum, cp_seg_blk)) {
			MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
			goto free_cp_payload;
		}

		// cold data stores SIT cache
		journal = &sum->journal;
		journal->n_sits = cpu_to_le16(6);
		journal->sit_j.entries[0].segno = cp->cur_node_segno[0 + i * NR_CURSEG_NODE_TYPE];
		journal->sit_j.entries[0].se.vblocks = cpu_to_le16((CURSEG_HOT_NODE << 10));
		journal->sit_j.entries[1].segno = cp->cur_node_segno[1 + i * NR_CURSEG_NODE_TYPE];
		journal->sit_j.entries[1].se.vblocks = cpu_to_le16((CURSEG_WARM_NODE << 10));
		journal->sit_j.entries[2].segno = cp->cur_node_segno[2 + i * NR_CURSEG_NODE_TYPE];
		journal->sit_j.entries[2].se.vblocks = cpu_to_le16((CURSEG_COLD_NODE << 10));

		journal->sit_j.entries[3].segno = cp->cur_data_segno[0 + i * NR_CURSEG_DATA_TYPE];
		journal->sit_j.entries[3].se.vblocks = cpu_to_le16((CURSEG_HOT_DATA << 10));
		journal->sit_j.entries[4].segno = cp->cur_data_segno[1 + i * NR_CURSEG_DATA_TYPE];
		journal->sit_j.entries[4].se.vblocks = cpu_to_le16((CURSEG_WARM_DATA << 10));
		journal->sit_j.entries[5].segno = cp->cur_data_segno[2 + i * NR_CURSEG_DATA_TYPE];
		...
		SET_SUM_TYPE((&sum->footer), SUM_TYPE_NODE);
		cp_seg_blk++;
		DBG(1, "\tWriting Segment summary for mlog %d HOT_NODE, at offset 0x%08"PRIx64"\n", i, cp_seg_blk);
		if (dev_write_block(sum, cp_seg_blk)) {
			MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
			goto free_cp_payload;
		}
		...
		SET_SUM_TYPE((&sum->footer), SUM_TYPE_NODE);
		cp_seg_blk++;
		DBG(1, "\tWriting Segment summary for mlog %d WARM_NODE, at offset 0x%08"PRIx64"\n", i, cp_seg_blk);
		if (dev_write_block(sum, cp_seg_blk)) {
			MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
			goto free_cp_payload;
		}
		...
		SET_SUM_TYPE((&sum->footer), SUM_TYPE_NODE);
		cp_seg_blk++;
		DBG(1, "\tWriting Segment summary for mlog %d COLD_NODE, at offset 0x%08"PRIx64"\n", i, cp_seg_blk);
		if (dev_write_block(sum, cp_seg_blk)) {
			MSG(1, "\tError: While writing the sum_blk to disk!!!\n");
			goto free_cp_payload;
		}
...
#ifdef FILE_CELL
	/* update node nat for Max*/
	for( i = get_sb(node_ino); i < NAT_ENTRY_PER_BLOCK; i++){
		nat_blk->entries[i].block_addr = cpu_to_le32(1);
		nat_blk->entries[i].ino = i;
	}
#else
	/* update node nat */
