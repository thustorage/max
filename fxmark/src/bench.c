...
//    FILE *numa_file = fopen("numa_scalibility.txt", "w+");
    /* default report_bench impl. */
    for (i = 0; i < bench->ncpu; ++i) {
        struct worker *w = &bench->workers[i];
        if (w->is_bg) continue;
//        fprintf(numa_file, "%d core, works: %lf\n", i, w->works);
        total_usecs += w->usecs;
        total_works += w->works;
    }
//    fclose(numa_file);
