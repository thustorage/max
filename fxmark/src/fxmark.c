...
static struct bench_desc bench_table[] = {
	...
	{"DWOL",
	 ...
	 &n_blk_wrt_ops},
	{"DWOL_pfsync",
	"block random write: each process randomly overwrites pages to a private file",
	&n_blk_wrt_pfsync_ops},
	{"DWOL_pfsync_pre",
	"block random write: prepare phase of DWOL_pfsync",
	&n_blk_wrt_pfsync_ops_pre},
	{"C_W_D",
...
static int parse_option(int argc, char *argv[], struct cmd_opt *opt)
{
	...
	for(arg_cnt = 0; 1; ++arg_cnt) {
		...
		c = getopt_long(argc, argv,
				"t:n:g:d:D:r:b:e:l:T:", options, &idx);
		if (c == -1)
			break;
		switch(c) {
		case 't':
...
static void usage(FILE *out)
{
	...
	fprintf(out, "  --times     = limited operation times\n");
}
