...
class Runner(object):
    ...
    MODULE_DIR = os.path.normpath(os.path.join(CUR_DIR, "../../Max/"))
    MKFS_DIR =  os.path.normpath(os.path.join(CUR_DIR, "../../disk-tools/mkfs/"))
    ...
    def __init__(self, \
        ...
        self.HOWTO_MOUNT = {
            ...
            "xfs_mem_jnl": self.mount_xfs_mem_jnl,
    ...
    def mount_max(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
        self.exec_cmd("rmmod max")
        self.exec_cmd("sudo insmod " + self.MODULE_DIR + "/" + fs + ".ko")
        p = self.exec_cmd("sudo " + self.MKFS_DIR + "/" + "mkfs.f2fs"
                          + " " + self.HOWTO_MKFS.get(fs, "")
                          + " " + dev_path,
                          self.dev_null)
        if p.returncode is not 0:
            return False
        p = self.exec_cmd(' '.join(["sudo mount -t max -o imds=72,mlog=8", 
                                    dev_path, mnt_path]), self.dev_null)
        if p.returncode is not 0:
            return False
    ...
    def mount_xfs_mem_jnl(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
        p = self.exec_cmd("sudo mkfs.xfs -f" + " " + dev_path + " -l logdev=/dev/ram0,size=1000m",
                          self.dev_null)
        if p.returncode is not 0:
            print("fail mkfs")
            return False
        p = self.exec_cmd(' '.join(["sudo mount -t xfs -o logdev=/dev/ram0", 
                                    dev_path, mnt_path]),
                          self.dev_null)
        if p.returncode is not 0:
            print("fail mount")
