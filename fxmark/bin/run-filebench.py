...
class FileBench(object):
    ...
    def __init__(self, type_, ncore_, duration_, root_,
        ...
        self.config = None
        self.bench_out = None
        self.filebench_path = os.path.normpath(os.path.join(CUR_DIR, "./"))
    ...
    def _run_filebench(self):
        with tempfile.NamedTemporaryFile(delete=False) as self.bench_out:
            cmd = "/usr/local/bin/filebench -f %s" % self.config.name
    ...
    def report(self):
        # 61.037: IO Summary: 60980518 ops 1016277.789 ops/s 156350/156350 rd/wr 3665.1mb/s 0.006ms/op
        work = 0
        work_sec = 0
    ...
    def setup_workload(self):
        ...
        elif self.workload == "fileserver" or self.workload == "webproxy" or self.workload == "webserver":
            core_per_chip = cpupol.CORE_PER_CHIP
            inc = int(core_per_chip / 2) if int(core_per_chip / 2) <= 10 else 8
            nfiles = 10000
            if self.workload == "webserver":
                nfiles = 10000
                inc = 9 
            if self.ncore >= inc:
                partition = self.ncore / inc
                threads = inc
                self._append_to_config("set $nfiles=%d" % (nfiles / partition + 1))
                cmd = "bash %s/gen_fileserver %d %d %d %s.tmp>> %s" % (self.filebench_path,
                partition, 1, threads, self.workload, self.config.name)
                self.ncore = int(partition * threads)
            else:
                self._append_to_config("set $nfiles=%d" % nfiles)
                cmd = "bash %s/gen_fileserver %d %d %d %s.tmp>> %s" % (self.filebench_path,
                1, 1, self.ncore, self.workload, self.config.name)
            self._exec_cmd(cmd).wait()
        elif self.workload == "varmail":  # varmail does not suffers futex
