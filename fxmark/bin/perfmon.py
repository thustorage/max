...
class PerfMon(object):
    ...
    def __init__(self, \
                 ...
                 iofile=os.environ.get('PERFMON_IOFILE', "_io.stat"), \
                 duration=30, \
                 media=os.environ.get('PERFMON_MEDIA', '')):
        (self.LEVEL, self.DIR, self.FILE) = (level, ldir, lfile)
        self.duration = duration
        self.cpu_stat = os.path.normpath(
            os.path.join(self.DIR, self.FILE))
        self.io_stat = iofile
        self.device = {"ssd": "sda", "hdd": "dm-3", "nvme": "nvme0n1", "mem": "loop0"}
        self.media = media
    ...
    def _cpu_stat_start(self):
        ...
        with open(self.cpu_stat, "w") as fd:
            ...
            fd.flush()
        # self._exec_cmd("rm -f  %s.tmp" % self.io_stat)
        # self._exec_cmd("iostat -d -x %s 2 >> %s.tmp &" % (self.device[self.media], self.io_stat))

    def _cpu_stat_stop(self):
        # p = self._exec_cmd("%s/kill-io-util.sh %s %s" % (CUR_DIR, self.io_stat, self.device[self.media]),
        #                    subprocess.PIPE)
        # io_stat = float(p.stdout.readline())
    ...
    def _perf_lock_record_start(self):
        self._exec_cmd("sudo sh -c \"echo 0 >/proc/lock_stat\"")
