...
class Plotter(object):
    ...
    def _plot_sc_data(self, media, bench, iomode):
        ...
        # gen gp file
        print("", file=self.out)
        print("set title \'%s:%s:%s\'" % (media, self.get_bench_title(bench), iomode), file=self.out)
    ...
    def plot_cmpdev(self, ncore, out_dir):
        ...
        self.data_file = ("cmpdev.%s.dat" % ncore)
        self.out = open(self.out_file, "w")
        ## TC
        ...
        ## print gp file
        self.out_file = os.path.join(self.out_dir, ("cmpdev.%s.gp" % ncore))
        self.out = open(self.out_file, "w")
        self._gen_log_info()
        ## plot header
        n_unit = len(self.config["bench"])
        n_col = min(n_unit, int(self.PAPER_WIDTH / self.UNIT_WIDTH))
        n_row = math.ceil(float(n_unit) / float(n_col))
        print("set term pdfcairo size %sin,%sin font \',10\'" %
              (self.UNIT_WIDTH * n_col, self.UNIT_HEIGHT * n_row),
    ...
    def _gen_cmpdev_for_bench(self, ncore, bench):
        ...
        print("# fs ssd-rel nvme-rel hdd ssd nvme", file=self.out)
