...
class Plotter(object):
    def __init__(self, log_file):
        ...
        self.CPU_UTILS = ["user.util",
                          ...
                          "idle.util",
                          "iowait.util"]
        self.MEDIA = ["hdd", "ssd", "nvme"]
        self.bench_title = {"C_W_D": "create-write-fsync-unlink",
                            "rocksdb_overwrite": "RocksDB"}
        self.UNIT_ADAPTIVE = {"dbench_client": {"nvme": 1000.0, "ssd": 1000},
                              "filebench_varmail": {"nvme": 1000.0, "ssd": 1000},
                              "rocksdb_overwrite": {"nvme": 1000, "ssd": 1000},
                              "filebench_fileserver": {"nvme": 1000000, "ssd": 1000000},
                              "filebench_webproxy": {"nvme": 1000000, "ssd": 1000000},
                              "C_W_D": {"nvme": 1000, "ssd": 1000, "hdd": 1000},
                              "MWCL": {"mem": 1000000},
                              "MWUL": {"mem": 1000000},
                              "DWAL": {"mem": 1000000}}
        self.LINE_STYLE = {"btrfs": 1,
                           "MAX": 2,
                           "ext4": 3,
                           "f2fs": 4,
                           "xfs": 6}
        self.Y_AXIS = {1000: "K", 1: "", 1000000: "M", 1000000000: "G"}
        self.UNIT = 1000000.0
    ...
    def get_bench_title(self, bench_name):
        title = self.bench_title.get(bench_name)
        if title is not None:
            return title
        return bench_name

    def get_unit(self, bench_name, media):
        unit_ = self.UNIT_ADAPTIVE.get(bench_name)
        if unit_ is not None:
            unit = unit_.get(media)
            if unit is not None:
                return unit
        return self.UNIT
    ...
    def _plot_header(self):
        ...
        print("set term pdfcairo size %sin,%sin font \'Times-Roman,10\'" %
    ...
    def _plot_sc_data(self, media, bench, iomode):
        def _get_sc_style(fs):
            if self.LINE_STYLE.get(fs) is not None:
                return "with lp ls %d lw 2 ps 0.8" % self.LINE_STYLE[fs]
            else:
                return "with lp lw 2 ps 0.8"
        ...
        def _get_data_file(fs):
            return "%s_%s_%s_%s.dat" % (media, fs, bench, iomode)
        ...
        for fs in fs_list:
            ...
            with open(data_file, "w") as out:
                print("# %s_%s_%s_%s_*" % (media, fs, bench, iomode), file=out)
                for d_kv in data:
                    d_kv = d_kv[1]
                    # if int(d_kv["ncpu"]) > self.ncore:
                    #     break
                    # print("%s %s %s %s %s" % (d_kv["ncpu"], float(d_kv["works/sec"]) / self.UNIT,
                    #                        float(d_kv["ioutil.util"]), fs,media))
                    print("%s %s" % (d_kv["ncpu"], float(d_kv["works/sec"]) / self.get_unit(bench, media)), file=out)
                    # print("%s %s %s" % (d_kv["ncpu"], float(d_kv["works/sec"]) / self.UNIT,
                    #                     float(d_kv["ioutil.util"])), file=out)

        # gen gp file
        print("", file=self.out)
        print("set title \'%s:%s\'" % (self.get_bench_title(bench), _get_media(media)), file=self.out)
        print("set xlabel \'# cores\'", file=self.out)
        print("set ylabel \'%s ops/sec' offset 1" % self.Y_AXIS[self.get_unit(bench, media)], file=self.out)
        fs = fs_list[0]
        print("plot [0:][0:] \'%s\' using 1:2 title \'%s\' %s"
              % (_get_data_file(fs), _get_fs(fs), _get_sc_style(fs)),
              end="", file=self.out)
        for fs in fs_list[1:]:
    ...
    def _plot_sc_ioutil_data(self, media, bench, iomode):
        def _get_data_file(fs):
            return "%s_%s_%s_%s.dat" % (media, fs, bench, iomode)
        ...
        # gen gp file
        print("", file=self.out)
        print("set title \'%s:%s\'" % (self.get_bench_title(bench), iomode), file=self.out)
        print("set xlabel \'# cores\'", file=self.out)
        print("set ylabel \'%s\'" % "IO utilization %", file=self.out)
        print("set key out horiz", file=self.out)
        print("set key center top", file=self.out)

        fs = fs_list[0]
        print("plot [0:][0:]\'%s\' using 1:3 title \'%s\' %s"
              % (_get_data_file(fs), fs, _get_sc_style(fs)),
              end="", file=self.out)
        for fs in fs_list[1:]:
            print(", \'%s\' using 1:3 title \'%s\' %s"
                  % (_get_data_file(fs), fs, _get_sc_style(fs)),
                  end="", file=self.out)
        print("", file=self.out)
    ...
    def _plot_cmpdev_data(self, ncore, bench, iomode):
        print("set style data histograms", file=self.out)
        print("set style fill solid 1.0 border -1", file=self.out)
        print("set title \'%s(%s core)\'" % (self.get_bench_title(bench), ncore), file=self.out)
        print("set xlabel \'\'", file=self.out)
        print("set ylabel \'Normalized Performance\'", file=self.out)
        print("set yrange [0:100]", file=self.out)
        # print("set key out horiz", file=self.out)
        # print("set key center top", file=self.out)
        print("# %s:*:%s:%s" % (bench, ncore, iomode), file=self.out)
        print("plot \'%s\' using 2:xtic(1) title \'%s\'" % (self.data_file, self.MEDIA[0]), end="", file=self.out)
    ...
    def _plot_cmpdev_mem_data(self, ncore, bench, iomode):
        print("set style data histograms", file=self.out)
        print("set style fill solid 1.0 border -1", file=self.out)
        print("set title \'%s(%s core)\'" % (self.get_bench_title(bench), ncore), file=self.out)
        print("set xlabel \'\'", file=self.out)
        print("set ylabel \'Memory level CPU utilization\'", file=self.out)
        # print("set key out horiz", file=self.out)
        # print("set key center top", file=self.out)
        print("# %s:*:%s:%s" % (bench, ncore, iomode), file=self.out)
        print("plot \'%s\' using 8:xtic(1) title \'%s\'" % (self.data_file, self.MEDIA[0]), end="", file=self.out)
        for (i, media) in enumerate(self.MEDIA[1:]):
            print(",\'\' using %s title \'%s\'"
                  % (i + 9, media),
                  end="", file=self.out)
    ...
    def _plot_cmpdev_io_data(self, ncore, bench, iomode):
        print("set style data histograms", file=self.out)
        print("set style fill solid 1.0 border -1", file=self.out)
        print("set title \'%s(%s core)\'" % (self.get_bench_title(bench), ncore), file=self.out)
        print("set xlabel \'\'", file=self.out)
        print("set ylabel \'IO Utilization %\'", file=self.out)
        print("set yrange [0:100]", file=self.out)
        # print("set key out horiz", file=self.out)
        # print("set key center top", file=self.out)
        print("# %s:*:%s:%s" % (bench, ncore, iomode), file=self.out)
        print("plot \'%s\' using 11:xtic(1) title \'%s\'" % (self.data_file, self.MEDIA[0]), end="", file=self.out)
        for (i, media) in enumerate(self.MEDIA[1:]):
            print(",\'\' using %s title \'%s\'"
                  % (i + 12, media),
                  end="", file=self.out)
    ...
    def plot_sc(self, out_dir):
        ...
        for bench in self.config["bench"]:
            for iomode in self.config["iomode"]:
                self.out_file = os.path.join(self.out_dir, "sc_%s_%s.gp" % (bench, iomode))
        ...
        #     for bench in self.config["bench"]:
        #         for iomode in self.config["iomode"]:
        #             self.out_file = os.path.join(self.out_dir, "sc_ioutil_%s_%s_%s.gp" % (media, bench, iomode))
        #             self.out = open(self.out_file, "w")
        #             self._gen_log_info()
        #             self._plot_header()
        #             self._plot_sc_ioutil_data(media, bench, iomode)
        #             self._plot_footer()
        #             self.out.close()
        #             self._gen_pdf(self.out_file)
    ...
    def plot_util(self, ncore, out_dir):
        ...
        for media in self.config["media"]:
            for bench in self.config["bench"]:
                for iomode in self.config["iomode"]:
                    self.out_file = os.path.join(self.out_dir, ("util_%s_%s_%s_%s.gp" % (media, bench, iomode, ncore)))
                    self.out = open(self.out_file, "w")
                    self._gen_log_info()
                    self._plot_header(bench, iomode)
                    self._plot_util_data(media, ncore, bench, iomode)
                    self._plot_footer()
                    self.out.close()
                    self._gen_pdf(self.out_file)

    def plot_cmpdev_header(self):
        n_unit = 1
        n_col = min(n_unit, int(self.PAPER_WIDTH / self.UNIT_WIDTH))
        n_row = math.ceil(float(n_unit) / float(n_col))
    ...
    def cmp_perf(self, ncore, out_dir):
        self.out_dir = out_dir
        subprocess.call("mkdir -p %s" % self.out_dir, shell=True)
        self.out_file = os.path.join(self.out_dir, ("cmp_perf_%s.gp" % ncore))
        self.out = open(self.out_file, "w")
        print("### %s" % ncore, file=self.out)
        print("## bench media MAX/F2FS MAX/EXT4", file=self.out)
        for bench in self.config["bench"]:
            for iomode in self.config["iomode"]:
                for media in self.config["media"]:
                    data_MAX = self.parser.search_data([media, "MAX", bench, ncore, iomode])
                    data_F2FS = self.parser.search_data([media, "f2fs", bench, ncore, iomode])
                    data_ext4 = self.parser.search_data([media, "ext4", bench, ncore, iomode])
                    iops_MAX = float(data_MAX[0][1]["works/sec"])
                    iops_F2FS = float(data_F2FS[0][1]["works/sec"])
                    iops_ext4 = float(data_ext4[0][1]["works/sec"])
                    print("%s %s %s %s" % (bench, media, iops_MAX/iops_F2FS, iops_MAX/iops_ext4), file=self.out)

    def plot_cmpdev(self, ncore, out_dir):
        self.out_dir = out_dir
        subprocess.call("mkdir -p %s" % self.out_dir, shell=True)
        self.out_file = os.path.join(self.out_dir, ("cmpdev_%s.dat" % ncore))
        self.data_file = ("cmpdev_%s.dat" % ncore)
        self.out = open(self.out_file, "w")
        ## TC
        # fs ssd-rel nvme-rel hdd ssd nvme
        for bench in self.config["bench"]:
        ...
        for bench in self.config["bench"]:
            for iomode in self.config["iomode"]:
                self.out_file = os.path.join(self.out_dir, ("cmpdev_%s_%s_%s.gp" % (ncore, bench, iomode)))
                self.out = open(self.out_file, "w")
                self._gen_log_info()
                self.plot_cmpdev_header()
                self._plot_cmpdev_data(ncore, bench, iomode)
                self._plot_footer()
                self.out.close()
                self._gen_pdf(self.out_file)
        ...
        for bench in self.config["bench"]:
            for iomode in self.config["iomode"]:
                self.out_file = os.path.join(self.out_dir, ("cmpdev_mem_%s_%s_%s.gp" % (ncore, bench, iomode)))
                self.out = open(self.out_file, "w")
                self._gen_log_info()
                self.plot_cmpdev_header()
                self._plot_cmpdev_mem_data(ncore, bench, iomode)
                self._plot_footer()
                self.out.close()
                self._gen_pdf(self.out_file)
        ...
        for bench in self.config["bench"]:
            for iomode in self.config["iomode"]:
                self.out_file = os.path.join(self.out_dir, ("cmpdev_io_%s_%s_%s.gp" % (ncore, bench, iomode)))
                self.out = open(self.out_file, "w")
                self._gen_log_info()
                self.plot_cmpdev_header()
                print("set key out horiz", file=self.out)
                print("set key center top", file=self.out)
                self._plot_cmpdev_io_data(ncore, bench, iomode)
                self._plot_footer()
                self.out.close()
                self._gen_pdf(self.out_file)
    ...
    def _gen_cmpdev_for_bench(self, ncore, bench):
        ...
        print("# fs hdd-rel ssd-rel nvme-rel hdd ssd nvme hdd-mem ssd-mem nvme-mem", file=self.out)
        for fs in self._get_fs_list("*", bench, "*"):
            ...
            for d_kv in data:
                ...
                dev_val[dev] = d_kv[1]
                # XXX: ugly [[[
            if dev_val.get("nvme", None) == None:
                print("WARNING: there is no %s:%s:%s:%s result." %
                      ("nvme", fs, bench, ncore), file=sys.stderr)
                continue
            ...
            nvme_perf = float(dev_val["nvme"]["works/sec"])
            nvme_mem = float(dev_val["nvme"]["sys.util"])
            nvme_io = float(dev_val["nvme"]["ioutil.util"])
            ssd_perf = float(dev_val["ssd"]["works/sec"])
            ssd_mem = float(dev_val["ssd"]["sys.util"])
            ssd_io = float(dev_val["ssd"]["ioutil.util"])
            hdd_perf = float(dev_val["hdd"]["works/sec"])
            hdd_mem = float(dev_val["hdd"]["sys.util"])
            hdd_io = float(dev_val["hdd"]["ioutil.util"])
            print("%s %s %s %s %s %s %s %s %s %s %s %s %s" %
                  (fs,
                   '1', ssd_perf / hdd_perf, nvme_perf / hdd_perf,
                   hdd_perf, ssd_perf, nvme_perf,
                   hdd_mem/hdd_perf, ssd_mem/ssd_perf, nvme_mem/nvme_perf,
                   hdd_io, ssd_io, nvme_io),
                  file=self.out)
            # XXX: ugly ]]]
        print("\n", file=self.out)
...
if __name__ == "__main__":
    ...
    parser.add_option("--ty", help="{sc | util | cmpdev | cmp }")
    ...
    if opts.ty == "sc":
        ...
        plotter.plot_cmpdev(int(opts.ncore), opts.out)
    elif opts.ty == "cmp":
        plotter.cmp_perf(int(opts.ncore), opts.out)
    else:
        __print_usage()
        exit(1)
